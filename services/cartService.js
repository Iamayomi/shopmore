...
exports.addToCart = async function (req, res, next) {
  ...
  try {
    ...
    const user = await User.findOne({ where: { id: req.user.id }, include: { model: Cart } });
    ...
    const existProduct = await CartItem.findOne({ where: { cartId: cart.id, productId: product.id }});
